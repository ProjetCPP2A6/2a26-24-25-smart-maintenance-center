...
{
    ui->setupUi(this);
    connect(ui->pushButtonSupprimerFacture, &QPushButton::clicked, this, &MainWindow::supprimerFacture);
    connect(ui->rechercherButton, &QPushButton::clicked, this, &MainWindow::rechercherFacture);
    connect(ui->pushButtonModifierFacture, &QPushButton::clicked, this, &MainWindow::modifierFacture);
    connect(ui->triantPrix, &QPushButton::clicked, this, &::MainWindow::on_triantprix_clicked);
    connect(ui->pushButtonSearchFournisseur, &QPushButton::clicked, this, &MainWindow::on_pushButtonSearchFournisseur_clicked);
    ...
    // Initialiser le modèle pour le tableau
        model = new QStandardItemModel(this);

        // Définir les colonnes du tableau
        model->setColumnCount(7); // 7 colonnes : ID, Date, Fournisseur, Quantité, Prix, Montant, Statut
        model->setHorizontalHeaderLabels({"ID", "Date", "Fournisseur", "Quantité", "Prix Produit", "Montant", "Statut"});

        // Lier le modèle au QTableView
        ui->tableViewFactures->setModel(model);

        // Connecter le bouton pushButtonAddFacture à la fonction
        connect(ui->pushButtonAddFacture, &QPushButton::clicked, this, &MainWindow::addFacture);
...
{
    QString fournisseur = ui->lineEditFournisseur_2->text();  // Récupérer le nom du fournisseur entré
    if (fournisseur.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Veuillez entrer un nom de fournisseur !");
        return;
    }

    // Initialiser une variable pour compter les occurrences
    int occurrences = 0;
    QStandardItemModel *model = qobject_cast<QStandardItemModel *>(ui->tableViewFactures->model());

    // Vérifier que le modèle existe
    if (model) {
        int rowCount = model->rowCount();

        // Parcourir les lignes du modèle pour chercher les occurrences du fournisseur
        for (int row = 0; row < rowCount; ++row) {
            QString currentFournisseur = model->item(row, 2)->text();  // Supposons que la colonne 2 contient les noms des fournisseurs
    ...
    // Calculer le pourcentage basé sur les occurrences
    int maxOccurrences = 4;  // Le nombre maximal d'occurrences avant d'atteindre 100%
    int percentage = (qMin(occurrences, maxOccurrences) * 100) / maxOccurrences;

    // Mettre à jour la ProgressBar
    ui->progressBarRegularite->setValue(percentage);
}

void MainWindow::on_triantprix_clicked()
{
    // Définir l'index de la colonne du prix
    int col =2 ; // Colonne correspondant au Prix unitaire

    // Déterminer l'ordre actuel et inverser
    Qt::SortOrder order = ui->tableViewFactures->horizontalHeader()->sortIndicatorOrder() == Qt::AscendingOrder ?
                          Qt::DescendingOrder : Qt::AscendingOrder;

    // Effectuer le tri par colonne
    ui->tableViewFactures->sortByColumn(col, order);
}

void MainWindow::modifierFacture()
{
    // Récupérer l'ID entré pour la modification
    QString factureID = ui->lineEditModifierFactureId_2->text();

    if (factureID.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Veuillez entrer un ID de facture pour la modifier !");
        return;
    }

    // Rechercher la facture par ID dans le modèle
    bool found = false;
    for (int row = 0; row < model->rowCount(); ++row) {
        QString currentID = model->item(row, 0)->text(); // Colonne 0 correspond à l'ID

        if (currentID == factureID) {
            // Modifier les valeurs dans le modèle
            model->setItem(row, 1, new QStandardItem(ui->lineEditModifierDate->text())); // Date
            model->setItem(row, 2, new QStandardItem(ui->lineEditModifierFournisseur->text())); // Fournisseur
            model->setItem(row, 3, new QStandardItem(ui->lineEditModifierQuantite->text())); // Quantité
            model->setItem(row, 4, new QStandardItem(ui->lineEditModifierPrixunitaire->text())); // Prix unitaire
            model->setItem(row, 5, new QStandardItem(ui->lineEditModifierMontant->text())); // Montant
            model->setItem(row, 6, new QStandardItem(ui->lineEditModifierStatut->text())); // Statut
    ...
    if (found) {
        QMessageBox::information(this, "Succès", "La facture a été modifiée avec succès !");
    } else {
        QMessageBox::warning(this, "Erreur", "Aucune facture trouvée avec cet ID !");
    }
    ui->lineEditModifierDate->clear();
    ui->lineEditModifierFournisseur->clear();
    ui->lineEditFournisseur->clear();
    ui->lineEditModifierQuantite->clear();
    ui->lineEditModifierPrixunitaire->clear();
    ui->lineEditModifierMontant->clear();
    ui->lineEditModifierStatut->clear();
}

void MainWindow::rechercherFacture()
{
    // Récupérer l'ID entré dans le champ de recherche
    QString searchID = ui->lineEditFournisseurSearch->text();
    ...
    for (int row = 0; row < model->rowCount(); ++row) {
        // Vérifier si l'ID de la ligne correspond à l'ID recherché
        QString currentID = model->item(row, 0)->text(); // Colonne 0 correspond à l'ID
        bool shouldShow = (currentID == searchID);

        // Afficher ou masquer la ligne dans le tableau
        ui->tableViewFactures->selectRow(row);
    }
}

void MainWindow::supprimerFacture()
{
    // Obtenir l'index de la ligne sélectionnée
    QModelIndexList selectedRows = ui->tableViewFactures->selectionModel()->selectedRows();
    ...
    // Supprimer chaque ligne sélectionnée
    for (const QModelIndex &index : selectedRows) {
        model->removeRow(index.row());
    }

    QMessageBox::information(this, "Succès", "La facture a été supprimée.");
}

void MainWindow::addFacture()
{
    // Récupérer les données des champs QLineEdit
    ...
    if (id.isEmpty() || date.isEmpty() || fournisseur.isEmpty() || quantite.isEmpty() || prixProduit.isEmpty() || montant.isEmpty() || statut.isEmpty()) {
    ...
    newRow << new QStandardItem(id)
           << new QStandardItem(date)
           << new QStandardItem(fournisseur)
           << new QStandardItem(quantite)
           << new QStandardItem(prixProduit)
           << new QStandardItem(montant)
...
//ressources
 ...
 void MainWindow::readFromArduino_2() {
